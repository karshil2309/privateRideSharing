...
$('document').ready(function() {
	...
	$('.table .btn-primary').on('click',function(event){		
		...
		$.get(href, function(RegistrationModel, userid){
			$('#idEdit').val(RegistrationModel.firstName);
			$('#ddlVehicleEdit').val(RegistrationModel.email);
			$('#ddlLocation1Edit').val(RegistrationModel.address);
			$('#ddlLocation2Edit').val(RegistrationModel.lastname);
			
			var date1 = vehicleMovement.date1.substr(0,10);
			var date2 = vehicleMovement.date2.substr(0,10);
			$('#date1Edit').val(dateOfBirth);
			$('#date2Edit').val(date2);
